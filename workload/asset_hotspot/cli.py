...
def main() -> None:
    ...
    with Path(args.config).open(encoding="utf-8") as stream: config = yaml.safe_load(stream)
    trace, meta = generate_from_config(config, args.output); print(f"wrote {trace}\nwrote {meta}")
