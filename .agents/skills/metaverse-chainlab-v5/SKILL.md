...
The canonical reader remains backward compatible with `mbe_workload_record_v1` and the Decentraland semantic-template `mbe_workload_record_v2`. New heterogeneous datasets use `mbe_workload_record_v3`, which carries a direct, digest-verified access list with only the generic modes `read`, `write`, `read_write`, and `commutative_delta`. Source-specific fields belong in dataset adapters or canonical record `metadata`; the compiler, Go iterator, routing path, and block executors consume only the common record contract. Registered adapters currently include Decentraland, Alien World RMW, Axie full-day transfer semantics, Tapos exact write sets, and generic canonical CSV.

Dataset files live under the repository-local ignored `dataset/` tree while Git tracks only manifests, schemas, samples, adapters, and tests. Single-file and indexed file-family layouts are both supported. Variant parameters and transaction-count choices are manifest-driven. Adapters must not inspect the selected execution method, and execution methods must not branch on dataset ID. A comparison group must reuse the same materialized workload identity and SHA-256 across Serial, Aria, Block-STM, Groundhog, MetaTrack, and later methods.

Paper Candidates using a dataset must record dataset ID, family/source-file hash where applicable, variant parameters, selection mode, materialized hash, realized skew, access-list schema/source, and actual cross-shard ratio. Keep the Go Interface + Factory Registry model. Do not modify consensus, finality definitions, or cross-shard protocol semantics unless a minimal replay adapter is proven necessary and tested.
