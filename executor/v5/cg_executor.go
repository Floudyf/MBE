...
func executeCGPlanWithCommitment(ctx context.Context, block realblock.Block, base map[string]string, baseCommitment *state.Commitment, plan literatureGraphPlan, workerCount int) (BlockExecutionResult, error) {
	...
	for _, wave := range plan.Waves {
		...
		snapshot := working
		started := time.Now()
		waveResults, observed, err := executeCGWave(ctx, block, wave, byID, indexByID, snapshot, workerCount)
	...
	for worker := 0; worker < workers; worker++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				results[task.index] = cgWaveResult{Item: item, Receipt: receipt, Delta: delta}
				mu.Lock()
				active--
				mu.Unlock()
			}
