...
func Run(input Input) (Result, error) {
	...
	if err := writeArtifacts(input.OutputDir, chainBytes, pluginBytes, experimentBytes, summary, blockLog, txResults, stateCommits, txPool.events, consensusLog, routingLog, executionLog, stateAccessLog, nodeRuntime, launcher, networkAdapter, consensusNetwork, pbftPreview, pbftNetwork, "V3.7.2 PBFT over NetworkAdapter Preview Closure run"); err != nil {
		return Result{}, err
	}
	return Result{OutputDir: input.OutputDir, Summary: summary, BlockLog: blockLog, TxResults: txResults, StateCommitLog: stateCommits, TxPoolLog: txPool.events, ConsensusLog: consensusLog, RoutingLog: routingLog, ExecutionLog: executionLog, StateAccessLog: stateAccessLog, NodeRuntime: nodeRuntime, Launcher: launcher, NetworkAdapter: networkAdapter, ConsensusNet: consensusNetwork, PBFTPreview: pbftPreview, PBFTNetwork: pbftNetwork, FinalState: state}, nil
}
