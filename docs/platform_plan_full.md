...
最终平台应以“实验范围 / 拓扑优先”的向导组织配置，而不是先平铺模板或算法配置。用户按以下顺序完成实验：

1. **选择实验范围**：single-chain、dual-chain、multi-chain 或 cross-chain protocol。
2. **配置链拓扑**：链数量、名称、类型、每条链的分片数、block interval、finality、吞吐目标、链间连接关系与异构链标记。
3. **配置每条链组件**：chain backend、共识、共识分片、状态分片、执行分片、路由、跨片协议、执行、提交、网络模型与指标。
4. **选择负载来源**：synthetic workload、chain-backed workload、existing trace replay 或 hybrid workload。
5. **配置 workload 参数**：tx count、交易混合、Zipf、热点、跨片/跨链比例、冲突、可交换更新、到达率、突发率与 seed。
6. **配置多链 / 跨链参数**：仅在适用范围中显示 source/target chain、跨链协议、finality wait、committee、timeout、pending pool 与批处理窗口。
7. **选择实验套件 / 策略组**：baseline、routing、execution、commit、ablation、MetaTrack 或 MetaFlow 比较。
8. **Composer Preview**：自动补齐默认组件，检查兼容性，标记 runnable / planned / invalid，并生成完整 config.yaml。
9. **运行**：trace-only、replay-only、full-pipeline、sweep 或 report-only。
10. **查看结果**：summary、latency、throughput、remote wait、rollback、aggregation、cross-chain、pending pool、figures 与 report。

当前阶段只开放已实现的范围与组件；其余步骤可展示为 planned，但不能被调度运行。
...
| V1：单链论文实验与向导骨架 | 以 single-chain 分片论文实验为主；V1.1 建立拓扑优先的实验向导与声明式配置骨架，后续阶段再接入论文机制、baseline、报告与小规模 Fabric 单链真实性校验。dual-chain、multi-chain、cross-chain protocol 仅作为 planned 能力展示。 | 支撑单链论文主实验，并保持 V0 闭环稳定。 |
| V2：多链与跨链实验平台 | 正式实现 dual-chain、multi-chain、异构链配置、跨链协议、committee bridge、Pending Pool、MetaFlow、跨链 workload 与指标面板。 | 支撑多链与跨链实验，而非仅展示规划。 |
