...
前端选择实验范围
  -> 配置单链 / 多链拓扑
  -> 配置每条链组件
  -> 选择负载来源并配置 workload 参数
  -> 选择实验套件 / 策略组
  -> Composer 生成完整配置并标记 runnable / planned / invalid
  -> 运行与结果展示
```
...
# V1.1：拓扑优先实验向导与声明式配置框架

## 目标

建立最终实验平台的向导式抽象：先确定实验范围、链拓扑、链内组件与负载来源，再选择策略组并进入 Composer Preview。当前只有 single-chain 可用；dual-chain、multi-chain 与 cross-chain protocol 只作为 planned 展示。唯一 runnable config 保持为 `v1_baseline_hash_serial`，其余 planned 配置不能运行。
...
在 V1.1 中，这些 API 服务于实验向导骨架：模板和配置是 Composer Preview 的声明来源，而不是要求用户先从平铺列表中选择算法。未来可选扩展 `wizard-options` 与 `wizard-preview`，但本阶段不要求实现。
...
```text
Step 1：实验范围（single_chain 可用；dual_chain / multi_chain / cross_chain_protocol 规划中）
Step 2：链拓扑（当前 1 条 mockchain，shard_count: 4；双链与异构链规划中）
Step 3：链内组件（consensus、分片、routing、execution、commit、metrics）
Step 4：负载来源（synthetic 与 existing trace replay 可用；Fabric chain-backed trace 规划中）
Step 5：实验套件 / 策略组（v1_execution_comparison 可预览；MetaTrack、ablation、Fabric 规划中）
Step 6：Composer Preview（显示 v1_baseline_hash_serial；其余 blockstm_like、calvin_like、porygon_like、ours、ablation、fabric 均为 planned）
```
...
V1 的主要可运行对象是 single-chain 论文实验。dual-chain、multi-chain 与 cross-chain protocol 在 V1 中仅保留 planned 展示和配置结构；Fabric chain-backed trace 仅用于小规模单链真实性校验，不正式实现跨链协议实验。
...
```text
正式 dual-chain / multi-chain 实验
异构链配置
跨链协议、committee bridge、Pending Pool、MetaFlow
cross_chain.csv、pending_pool.csv 等跨链指标
```
...
```text
打开前端
  -> 选择实验范围：single_chain / dual_chain / multi_chain / cross_chain_protocol
  -> 配置链拓扑与链组件
  -> 选择负载来源与 workload 参数
  -> 选择实验套件 / 策略组
  -> Composer Preview
