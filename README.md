...
Current status: V3.4.11 closure. The latest runtime capability is the V3.4.10 controlled MetaTrack smoke runner over a local Go-backed modular research chain Draft Smoke runtime. V3.4.9 introduced the MetaTrack ablation templates; V3.4.10 introduced the controlled smoke runner; V3.4.11 aligns stage/version fields, frontend wording, repository docs, Codex skill guidance, and validation.

The project is not yet a real multi-node blockchain, not a Fabric/EVM live backend, not a BlockEmulator backend, not real PBFT/HotStuff/Raft, not a real cross-shard protocol implementation, and not a paper-grade benchmark. The next planned stage is V3.5 node-level emulator skeleton.
