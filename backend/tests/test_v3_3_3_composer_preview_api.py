...
def test_v3_composer_preview_api_returns_additive_preview_fields() -> None:
    ...
    assert payload["stage"] == "V3.5.4"
    assert payload["current_stage"] == "V3.5.4"
    assert payload["latest_runtime_stage"] == "V3.5 local node process preview runtime"
    assert payload["latest_completed_runtime_stage"] == "V3.5 local node process preview runtime"
    assert payload["current_capability"] == "configurable logical node topology, launcher preview artifacts, and local node process preview entry point"
    assert payload["runtime_truth"] == "local_node_process_preview_not_real_tcp_not_real_pbft"
    assert payload["next_stage"] == "V3.6 TCP Adapter and Consensus Hardening"
