...
def test_v3_composer_preview_api_returns_additive_preview_fields() -> None:
    ...
    assert payload["stage"] == "V3.9 State Authenticity Layer MVP Closure"
    assert payload["current_stage"] == "V3.9 State Authenticity Layer MVP Closure"
    assert payload["latest_runtime_stage"] == "persistent state backend with Merkle/MPT-like state root, proof verification, and stateless witness artifacts"
    assert payload["latest_completed_runtime_stage"] == "persistent state backend with Merkle/MPT-like state root, proof verification, and stateless witness artifacts"
    assert payload["current_capability"] == "persistent state backend, deterministic state roots, proof verification, and stateless witness artifacts under StateAccess / StateStorage / Commit"
    assert payload["runtime_truth"] == "state_authenticity_mvp_not_ethereum_compatible_mpt_or_full_stateless_execution"
    assert payload["next_stage"] == "V3.10 Benchmark / Experiment Template Hardening"
