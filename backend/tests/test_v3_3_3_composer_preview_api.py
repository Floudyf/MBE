...
def test_v3_composer_preview_api_returns_additive_preview_fields() -> None:
    ...
    assert payload["stage"] == "V3.7.2 V3.7 Closure"
    assert payload["current_stage"] == "V3.7.2 V3.7 Closure"
    assert payload["latest_runtime_stage"] == "configurable ConsensusRuntime with BlockEmulator-aligned PBFT preview over NetworkAdapter"
    assert payload["latest_completed_runtime_stage"] == "configurable ConsensusRuntime with BlockEmulator-aligned PBFT preview over NetworkAdapter"
    assert payload["current_capability"] == "BlockEmulator-aligned PBFT preview over selected NetworkAdapter typed message path"
    assert payload["runtime_truth"] == "blockemulator_aligned_pbft_preview_over_network_not_production_pbft"
    assert payload["next_stage"] == "V3.8 CrossShardProtocol Skeleton"
