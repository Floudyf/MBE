...
def test_v3_composer_preview_api_returns_additive_preview_fields() -> None:
    ...
    assert payload["stage"] == "V3.10 Benchmark / Experiment Template Hardening Closure"
    assert payload["current_stage"] == "V3.10 Benchmark / Experiment Template Hardening Closure"
    assert payload["latest_runtime_stage"] == "benchmark template catalog, baseline profile catalog, local sweep runner, reproducibility manifest, and benchmark report artifacts"
    assert payload["latest_completed_runtime_stage"] == "benchmark template catalog, baseline profile catalog, local sweep runner, reproducibility manifest, and benchmark report artifacts"
    assert payload["current_capability"] == "benchmark template catalog, baseline profile catalog, local controlled sweep runner, repeatability manifest, and benchmark report artifacts"
    assert payload["runtime_truth"] == "benchmark_template_hardening_not_paper_grade_benchmark"
    assert payload["next_stage"] == "V3.11 CrossShard Protocol Hardening"
