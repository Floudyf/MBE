...
def test_v3_composer_preview_api_returns_additive_preview_fields() -> None:
    ...
    assert payload["stage"] == "V3.8 CrossShardProtocol Skeleton Closure"
    assert payload["current_stage"] == "V3.8 CrossShardProtocol Skeleton Closure"
    assert payload["latest_runtime_stage"] == "configurable CrossShardProtocol skeleton with relay_preview artifacts"
    assert payload["latest_completed_runtime_stage"] == "configurable CrossShardProtocol skeleton with relay_preview artifacts"
    assert payload["current_capability"] == "cross-shard transaction detection preview plus relay_preview skeleton artifacts under Routing/Sharding"
    assert payload["runtime_truth"] == "cross_shard_protocol_skeleton_not_atomic_cross_shard_commit"
    assert payload["next_stage"] == "V3.9 StateStorage / StateProof Hardening"
