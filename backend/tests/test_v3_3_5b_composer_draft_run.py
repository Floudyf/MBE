...
def test_run_draft_smoke_writes_single_draft_artifacts(monkeypatch, tmp_path: Path) -> None:
    ...
    def fake_run_go_v3_runtime(**kwargs):
        ...
        (output_dir / "summary.json").write_text('{"tx_count": 1, "success_count": 1}\n', encoding="utf-8")
        return SimpleNamespace(output_dir=output_dir, summary={"tx_count": 1, "success_count": 1})
    ...
    assert {"composer_draft.json", "normalized_draft.json", "generated_experiment_profile.json", "summary.csv", "runtime.log"} <= artifact_names
