...
CATALOG: dict[str, V3ModuleCapability] = {
    ...
    "Consensus": module(
        ...
        plugins=[
            ...
            runnable("blockemulator_aligned_pbft_preview", "BlockEmulator-aligned PBFT over NetworkAdapter preview"),
            ...
            planned("future_hotstuff_preview", "Future HotStuff preview planned"),
            planned("hotstuff", "HotStuff unsupported"),
            planned("raft_model", "Raft 模型"),
            planned("future_raft_preview", "Future Raft preview planned"),
    ...
    "Commit": module(
        ...
        plugins=[
            ...
            runnable("conservative_commit", "保守提交 light model"),
            runnable("hot_update_aggregation", "热点更新聚合 light model"),
            runnable("constraint_checked_aggregation", "约束检查聚合 light model"),
            runnable("hot_update_aggregation_commit", "热点更新聚合提交（兼容别名）"),
            planned("atomic_reservation_commit", "原子预留提交"),
            planned("batch_commit", "批量提交"),
            planned("real_db_lock_commit", "真实数据库锁提交"),
        ],
    ),
...
SINGLE_MODULE_TEMPLATE_CATALOG: dict[str, dict[str, Any]] = {
    ...
    "single_module_consensus": {
        ...
        "allowed_variable_plugins": ["simple_leader", "poa_light", "pbft_light_model", "blockemulator_aligned_pbft_preview"],
        ...
        "truthfulness_note": "pbft_light_model is PBFT-style stage/quorum/message-count model only. blockemulator_aligned_pbft_preview is a deterministic PBFT preview over the selected NetworkAdapter path. Neither is production PBFT or full Byzantine safety.",
        "default_preset_id": "consensus_light_smoke",
        "presets": [
            {
                "preset_id": "consensus_light_smoke",
                "preset_name": "Consensus-light smoke",
                "description": "Runs a local Draft Smoke that focuses on simple_leader, PoA-light, PBFT-light, or optional PBFT preview over NetworkAdapter metrics.",
                "default_chain_profile": "single_chain_research_default",
                "default_plugin_selection": dict(SINGLE_MODULE_LOCKED_PLUGIN_VALUES),
                "variable_module": "Consensus",
                ...
                "expected_artifacts": ["consensus_log.csv", "pbft_state_log.csv", "pbft_message_log.csv", "quorum_log.csv", "finalized_block_log.csv", "consensus_network_log.csv", "pbft_network_summary.json", "block_log.csv", "summary.csv", "summary.json", "tx_results.csv"],
                "result_guide": "Focus on consensus runtime selection, consensus latency, message count, PBFT preview stage/quorum artifacts, PBFT over NetworkAdapter counts when selected, and consensus_log.csv.",
                "truthfulness_note": "blockemulator_aligned_pbft_preview is a deterministic preview over NetworkAdapter typed messages. It is not production PBFT, not full Byzantine safety, and not a BlockEmulator backend.",
            }
        ],
    },
    "single_module_routing": {
        ...
        "truthfulness_note": "Routing light models estimate shard assignment and routing decisions. V3.11 keeps cross_shard_protocol as a Routing/Sharding sub-capability and adds relay_mvp artifacts for SourceLock, RelayCertificate, target commit, source finalization, timeout/refund/abort observability. It is not production atomic commit, complete Broker/2PC/Monoxide, Byzantine-secure relay, CLPA, ShardCutter, or a production cross-chain bridge.",
        ...
        "presets": [
            {
                ...
                "expected_artifacts": ["routing_log.csv", "cross_shard_tx_log.csv", "cross_shard_message_log.csv", "relay_preview_log.csv", "cross_shard_status.csv", "cross_shard_summary.json", "relay_state_machine_log.csv", "source_lock_log.csv", "relay_certificate_log.csv", "relay_proof_verification_log.csv", "target_commit_log.csv", "source_finalize_log.csv", "cross_shard_timeout_refund_log.csv", "cross_shard_failure_log.csv", "relay_mvp_summary.json", "summary.csv", "summary.json", "block_log.csv", "tx_results.csv"],
                "result_guide": "Focus on routing_plugin, cross_shard_ratio, touched shard counts, hotspot/co-access counts, cross_shard_protocol_selected, and V3.11 Relay MVP artifacts.",
                "truthfulness_note": "This preset validates routing/sharding decision behavior and optional relay_mvp artifacts. It is not real cross-shard production behavior, not production atomic cross-shard commit, not complete Relay/Broker/2PC/Monoxide, not Byzantine-secure relay, not a production cross-chain bridge, and not state migration.",
            }
        ],
    },
    ...
    "single_module_commit": {
        "template_id": "single_module_commit",
        "template_name": "Single-module Commit",
        "stage": "V3.4.8",
        "chain_mode": "single_chain",
        "status": "runnable",
        "runnable": True,
        "preview_only": False,
        "description": "Fair single-module template for Commit runtime hardening checks. Only Commit may vary.",
        "variable_module": "Commit",
        "allowed_variable_plugins": ["normal_commit", "conservative_commit", "hot_update_aggregation", "constraint_checked_aggregation"],
        "locked_plugin_values": {key: value for key, value in SINGLE_MODULE_LOCKED_PLUGIN_VALUES.items() if key != "Commit"},
        "fairness_rule": "Only CommitPlugin / Commit may vary; all other modules, workload, seed, submit rate, routing config, execution config, state access config, consensus config, and network profile stay fixed.",
        "truthfulness_note": "Commit light models estimate deterministic commit paths, hot-update aggregation, and constraint checks only. They do not implement real database locking, real concurrent commit, persistent state-tree validation, state root, or snapshots.",
        "default_preset_id": "commit_hot_update_smoke",
        "presets": [
            {
                "preset_id": "commit_hot_update_smoke",
                "preset_name": "Commit hot-update smoke",
                "description": "Runs a local Draft Smoke that focuses on state_commit_log records, hot-update aggregation, constraint checks, and commit latency.",
                "default_chain_profile": "single_chain_research_default",
                "default_plugin_selection": dict(SINGLE_MODULE_LOCKED_PLUGIN_VALUES) | {"Commit": "hot_update_aggregation"},
                "variable_module": "Commit",
                "locked_modules": {key: value for key, value in SINGLE_MODULE_LOCKED_PLUGIN_VALUES.items() if key != "Commit"},
                "primary_metrics": [
                    "aggregation_ratio",
                    "hotspot_update_count",
                ...
                "secondary_metrics": ["commit_plugin", "commit_tx_count", "commit_update_count", "raw_update_count", "aggregation_group_count", "max_commit_latency_ms"],
                "expected_artifacts": ["state_commit_log.csv", "state_access_log.csv", "execution_log.csv", "routing_log.csv", "summary.csv", "summary.json", "block_log.csv", "tx_results.csv"],
                "result_guide": "Focus on aggregation ratio, hotspot/aggregated updates, constraint checks, commit latency, and state_commit_log.csv.",
                "truthfulness_note": "This preset validates deterministic commit behavior in the local Go-backed runtime. It models hot-update aggregation and constraint checks, but does not implement real database locking, real concurrent commit, or persistent state-tree validation.",
            }
        ],
    },
}
