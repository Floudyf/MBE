    ...
    metadata = manager.create_run(
        ...
        extra_metadata={
            ...
            **stage_metadata(),
            ...
            "ablation_stage": normalized.get("ablation_stage", ""),
            "enabled_metatrack_components": normalized.get("enabled_metatrack_components", []),
            "controlled_experiment_enabled": bool(normalized.get("controlled_experiment_enabled", False)),
            "plugin_selection_mode": normalized.get("plugin_selection_mode", "free"),
            "controlled_modules": normalized.get("controlled_modules", []),
            ...
            "primary_metrics": normalized.get("primary_metrics", []),
...
def build_experiment_profile(normalized: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "process_runtime_mode": normalized.get("topology", {}).get("process_runtime_mode", "dry_run"),
        "max_local_processes": normalized.get("topology", {}).get("max_local_processes", 8),
        "enable_committee_epoch": normalized.get("topology", {}).get("enable_committee_epoch", True),
        "epoch_count": normalized.get("topology", {}).get("epoch_count", 1),
        "controlled_experiment_enabled": bool(normalized.get("controlled_experiment_enabled", False)),
        "plugin_selection_mode": normalized.get("plugin_selection_mode", "free"),
        ...
        "benchmark_template": normalized.get("topology", {}).get("benchmark_template", "full_stack_v3_template"),
        "baseline_profile": normalized.get("topology", {}).get("baseline_profile", "baseline_simple_chain"),
        "repeat_count": normalized.get("topology", {}).get("repeat_count", 1),
        "metaverse_suite_enabled": normalized.get("topology", {}).get("metaverse_suite_enabled", False),
        "metaverse_scenario": normalized.get("topology", {}).get("metaverse_scenario", "mixed_metaverse"),
        "topology": normalized.get("topology", {}),
        "topology_summary": normalized.get("topology_summary", {}),
        ...
        **stage_metadata(),
        ...
        "aggregation_candidates_enabled": normalized.get("plugin_selection", {}).get("Commit") in {"hot_update_aggregation", "hot_update_aggregation_commit", "constraint_checked_aggregation"},
...
def build_plugin_profile(normalized: dict[str, Any]) -> dict[str, Any]:
    ...
    consensus_selection = selection.get("Consensus", "simple_leader")
    if consensus_selection == "blockemulator_aligned_pbft_preview":
        plugins["ConsensusPlugin"] = "pbft_light_model"
        plugins["ConsensusRuntimePlugin"] = "blockemulator_aligned_pbft_preview"
    else:
        plugins["ConsensusRuntimePlugin"] = consensus_selection
    return {
