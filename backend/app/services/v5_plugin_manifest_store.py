...
_ZH = {
    ...
    "porygon_stateless_routing": ("Porygon 无状态路由", "按确定性状态归属从存储侧获取状态，并保持交易单次执行语义。"),
    "porygon_transaction_block_producer": ("Porygon 交易块装配器", "构造带交易根、访问根与完整数据可用性承诺的 Transaction Block。"),
    "porygon_execution": ("Porygon 执行分类", "标记 Porygon 全局排序后由执行子委员会处理的交易。"),
    "porygon_pipeline_scheduler": ("Porygon 三维并行规划器", "构造 Witness/Ordering/Execution/Commit 协议证据、ESC 分片计划与跨片单次执行计划。"),
    "porygon_block_executor": ("Porygon 分片执行器", "按共识绑定的 ESC/wave 计划并行执行，并确定性物化跨片多状态更新。"),
    "porygon_remote_state_access": ("Porygon 远程状态访问", "Stateless 节点通过 MBE 远程状态通路获取权威存储侧状态。"),
    "porygon_cross_shard_coordinator": ("Porygon 跨片协调", "按 Single-Shard Execution / Multi-Shard Update 语义记录锁定、单次执行与原子更新。"),
...
_manifest("block_executor", "cg_block_executor", "CG / Nezha MBE Worker Block Executor", "Executes only accepted dependency-ready frontiers from the consensus-bound Nezha-CG plan using the requested MBE worker count. Cycle victims remain algorithmic abort evidence but are non-terminal FIFO Deferred retries; no failed no-op receipt is emitted.", config={"worker_count": 4}, schema=_schema({"worker_count": {"type": "integer", "minimum": 1, "maximum": 64, "default": 4}}), capabilities=["parallel_execution_adaptation", "dependency_ready_frontiers", "deterministic_materialization", "deterministic_cycle_abort_evidence", "fifo_deferred_retry_lifecycle"], supported_backends=["real_cluster"], truth_boundary="nezha_cg_official_multigraph_bounded_johnson_retry_mbe_worker_v4"),
    ...
    # MBE_PORYGON_PAPER_REPRO_20260920_V7: Porygon paper-driven independent reimplementation.
    _manifest(
        "routing", "porygon_stateless_routing", "Porygon Stateless Routing",
        "Deterministic stateless remote-home routing for the Porygon reproduction.",
        capabilities=["batch_routing", "stateless_direct_execution", "remote_state_fetch", "remote_state_writeback"],
        supported_backends=["real_cluster"],
        truth_boundary="porygon_storage_consensus_separation_over_mbe_remote_state_v1",
    ...
    _manifest(
        ...
        "Builds a Transaction Block and binds transaction/access roots plus a data-availability witness commitment.",
        config={"block_size": 100, "transaction_block_size": 100, "interval_ms": 75, "witness_threshold": 1},
        ...
        capabilities=["porygon_transaction_block", "transaction_root", "access_root", "full_body_data_availability_commitment", "proposal_evidence"],
        ...
        supported_backends=["real_cluster"],
        truth_boundary="porygon_transaction_block_full_body_validator_recompute_v1",
        source={"source_type": "paper_reimplementation", "source_name": "Porygon: Scaling Blockchain via 3D Parallelism", "doi": "10.1109/ICDE60146.2024.00153", "reproduction_dossier": "docs/reproductions/porygon/source_lock.md"},
    ),
    _manifest(
        "execution", "porygon_execution", "Porygon Execution Classifier",
        "Execution classification for globally ordered Porygon transactions assigned to execution sub-committees.",
        capabilities=["porygon_global_order", "execution_subcommittee"], supported_backends=["real_cluster"],
        truth_boundary="porygon_global_order_then_esc_execution_v1",
        source={"source_type": "paper_reimplementation", "source_name": "Porygon: Scaling Blockchain via 3D Parallelism", "doi": "10.1109/ICDE60146.2024.00153", "reproduction_dossier": "docs/reproductions/porygon/source_lock.md"},
    ),
    _manifest(
        "scheduler", "porygon_pipeline_scheduler", "Porygon 3D Parallelism Planner",
        "Binds deterministic W/O/E/M protocol slots, Cross-Batch Witness evidence, ESC assignments, conflict-safe execution waves, and cross-shard single-execution plans before consensus.",
        ...
        capabilities=["porygon_witness_order_execute_commit", "cross_batch_witness", "execution_subcommittee_sharding", "consensus_bound_execution_plan", "single_shard_cross_shard_execution"],
        ...
        supported_backends=["real_cluster"],
        truth_boundary="porygon_3d_parallelism_plan_v1;pipeline_slots_are_protocol_evidence_not_wall_clock_overlap",
        source={"source_type": "paper_reimplementation", "source_name": "Porygon: Scaling Blockchain via 3D Parallelism", "doi": "10.1109/ICDE60146.2024.00153", "reproduction_dossier": "docs/reproductions/porygon/source_lock.md"},
    ),
    _manifest(
        "block_executor", "porygon_block_executor", "Porygon Sharded Block Executor",
        "Executes conflict-safe ESC waves from the consensus-bound Porygon plan; each cross-shard transaction executes once and its writes are deterministically materialized atomically.",
        config={"worker_count": 4},
        schema=_schema({"worker_count": {"type": "integer", "minimum": 1, "maximum": 32, "default": 4}}),
        capabilities=["porygon", "execution_subcommittee", "parallel_waves", "single_execution_cross_shard", "deterministic_multi_state_update", "execution_plan_digest"],
        ...
        metrics=[
            {"key": "porygon_witnessed_block_count", "type": "integer", "unit": "block", "aggregation": "sum", "visualization": "summary", "description": "Transaction blocks whose full body and roots were validated."},
            {"key": "porygon_cross_batch_witness_count", "type": "integer", "unit": "stage", "aggregation": "sum", "visualization": "summary", "description": "Logical Cross-Batch Witness protocol stages bound by Porygon plans."},
            {"key": "porygon_execution_shard_count", "type": "integer", "unit": "shard", "aggregation": "max", "visualization": "summary", "description": "Configured execution sub-committee shard count."},
            {"key": "porygon_cross_shard_transaction_count", "type": "integer", "unit": "tx", "aggregation": "sum", "visualization": "summary", "description": "Transactions touching more than one Porygon execution shard."},
            {"key": "porygon_single_shard_execution_count", "type": "integer", "unit": "tx", "aggregation": "sum", "visualization": "summary", "description": "Cross-shard transactions executed exactly once by one ESC."},
        ...
        supported_backends=["real_cluster"],
        truth_boundary="porygon_esc_parallel_execution_single_cross_shard_execution_v1",
        source={"source_type": "paper_reimplementation", "source_name": "Porygon: Scaling Blockchain via 3D Parallelism", "doi": "10.1109/ICDE60146.2024.00153", "reproduction_dossier": "docs/reproductions/porygon/source_lock.md"},
    ),
    _manifest(
        "state_access", "porygon_remote_state_access", "Porygon Remote State Access",
        "Uses MBE's real remote-state fetch/writeback transport for Porygon stateless execution; stateless execution nodes do not use direct local business-state access as the method contract.",
        capabilities=["remote_state_fetch", "remote_state_writeback", "exact_state_version_fetch"],
        requirements=["routing:porygon_stateless_routing"], supported_backends=["real_cluster"],
        truth_boundary="porygon_stateless_remote_state_access_over_mbe_transport_v1",
        source={"source_type": "paper_reimplementation", "source_name": "Porygon: Scaling Blockchain via 3D Parallelism", "doi": "10.1109/ICDE60146.2024.00153", "reproduction_dossier": "docs/reproductions/porygon/source_lock.md"},
    ),
    _manifest(
        "cross_shard", "porygon_cross_shard_coordinator", "Porygon Cross-shard Coordinator",
        "Models Porygon state locking, Single-Shard Execution, Multi-Shard Update, and unlock/abort evidence while direct stateless execution performs the transaction only once.",
        config={"execution_shard_count": 4}, schema=_schema({"execution_shard_count": {"type": "integer", "minimum": 1, "maximum": 64, "default": 4}}),
        capabilities=["porygon_state_lock", "single_shard_execution", "multi_shard_update", "cross_shard_atomicity"],
        requirements=["routing:porygon_stateless_routing", "block_executor:porygon_block_executor"], supported_backends=["real_cluster"],
        truth_boundary="porygon_single_execution_deterministic_multi_key_atomic_commit_v1",
        source={"source_type": "paper_reimplementation", "source_name": "Porygon: Scaling Blockchain via 3D Parallelism", "doi": "10.1109/ICDE60146.2024.00153", "reproduction_dossier": "docs/reproductions/porygon/source_lock.md"},
    ),
]
