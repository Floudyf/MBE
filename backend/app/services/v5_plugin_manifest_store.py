...
_ZH = {
    ...
    "cg_scheduler": ("CG 冲突图调度", "按 Nezha 官方 CG 基线构造完整读写冲突图，使用 JohnsonCE 周期成员规则中止事务后生成拓扑执行层。"),
    "cg_block_executor": ("CG 并行执行器", "按 Nezha 官方 CG 消环后的拓扑层并行执行，并记录 JohnsonCE 中止事务。"),
...
_manifest("execution", "cg_execution", "CG Execution", "Nezha official-reference conventional CG baseline classifier.", capabilities=["paper_baseline_cg", "full_pairwise_conflict_detection", "cycle_detection", "nezha_johnson_cycle_abort"], supported_backends=["real_cluster"], truth_boundary="nezha_cg_official_reference_johnson_v2", source={"source_type": "paper_and_official_source_reimplementation", "source_name": "Nezha: Exploiting Concurrency for Transaction Processing in DAG-based Blockchains", "doi": "10.1109/ICDCS54860.2022.00034", "official_repository": "CGCL-codes/Nezha", "official_commit": "85eaf541591e5f3020dd520cf3b8ee35009d296a", "reference_files": ["core/classical_graph.go", "graph/johnsonce.go", "graph/tarjanscc.go", "test.go"], "reference_scope": "conventional CG baseline shipped with the Nezha artifact", "cycle_resolution": "Tarjan SCC plus one JohnsonCE blocked/unblock elementary-cycle enumeration per original SCC; BreakCycles aborts maximum-membership vertex; lowest original ordinal on ties; no retry in reference benchmark", "implementation_note": "Sparse exact boolMap representation: compact cycle-member storage plus per-vertex cycle index; cycle set is enumerated once per SCC and never re-enumerated after a victim; representation-only adaptation, no heuristic or runtime feedback"}),
_manifest("scheduler", "cg_scheduler", "CG Scheduler", "Constructs Nezha's conventional directed RW/WW conflict graph, performs one official JohnsonCE cycle enumeration per SCC, applies BreakCycles maximum-membership victims, and derives residual topological execution waves.", capabilities=["consensus_bound_schedule", "full_pairwise_conflict_detection", "cycle_detection", "nezha_johnson_cycle_abort", "topological_waves"], requirements=["execution:cg_execution", "block_executor:cg_block_executor"], supported_backends=["real_cluster"], truth_boundary="nezha_cg_official_reference_johnson_v2"),
_manifest("block_executor", "cg_block_executor", "CG Block Executor", "Executes each residual Nezha-CG DAG frontier in parallel and terminalizes JohnsonCE-aborted transactions as failed no-op outcomes.", config={"worker_count": 4}, schema=_schema({"worker_count": {"type": "integer", "minimum": 1, "maximum": 64, "default": 4}}), capabilities=["parallel_execution", "deterministic_materialization", "deterministic_cycle_abort_receipts"], supported_backends=["real_cluster"], truth_boundary="nezha_cg_official_reference_johnson_v2"),
    _manifest("execution", "acg_execution", "ACG Execution", "Official-reference Nezha address-conflict-graph and hierarchical-sorting classifier.", capabilities=["paper_baseline_acg", "address_conflict_queues", "hierarchical_sorting", "deterministic_hs_abort_evidence"], supported_backends=["real_cluster"], truth_boundary="nezha_acg_hs_official_reference_v1", source={"source_type": "paper_and_official_source_reimplementation", "source_name": "Nezha: Exploiting Concurrency for Transaction Processing in DAG-based Blockchains", "doi": "10.1109/ICDCS54860.2022.00034", "official_repository": "CGCL-codes/Nezha"}),
    _manifest("scheduler", "acg_scheduler", "ACG Scheduler", "Builds one conflict queue per address, ranks address dependencies, and applies Nezha DeSS hierarchical sorting with explicit HS-abort evidence.", capabilities=["consensus_bound_schedule", "address_conflict_queues", "hierarchical_sorting", "deterministic_hs_abort_evidence"], requirements=["execution:acg_execution", "block_executor:acg_block_executor"], supported_backends=["real_cluster"], truth_boundary="nezha_acg_hs_official_reference_v1"),
    _manifest("block_executor", "acg_block_executor", "ACG Block Executor", "Executes consensus-bound Nezha HS sequence waves in parallel and terminalizes HS-aborted transactions as deterministic failed no-op outcomes.", config={"worker_count": 4}, schema=_schema({"worker_count": {"type": "integer", "minimum": 1, "maximum": 64, "default": 4}}), capabilities=["parallel_execution", "deterministic_materialization", "deterministic_hs_abort_receipts"], supported_backends=["real_cluster"], truth_boundary="nezha_acg_hs_official_reference_v1"),
