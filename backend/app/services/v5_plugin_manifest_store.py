...
_ZH = {
    ...
    "cg_scheduler": ("CG 冲突图调度", "构造完整读写冲突图，检测依赖环并确定性中止环中事务后生成拓扑执行层。"),
    "cg_block_executor": ("CG 并行执行器", "按消环后的冲突图拓扑层并行执行，并记录周期中止事务。"),
...
_MANIFESTS = [
    ...
    _manifest("execution", "cg_execution", "CG Execution", "Cycle-aware full pairwise conflict-graph baseline classifier used for the Batch-SI comparison.", capabilities=["paper_baseline_cg", "full_pairwise_conflict_detection", "cycle_detection", "deterministic_cycle_abort"], supported_backends=["real_cluster"], truth_boundary="cg_cycle_aware_conflict_graph_abort_v4", source={"source_type": "paper_and_reference_source_reimplementation", "source_name": "Batch-SI paper-level independent CG reproduction; Definition 2 calls CG a DAG while Sections 5.2/5.7 also require cycle handling and report CG aborts; no exact victim rule is published", "cited_reference_repository": "https://github.com/PDCRL/ConcSawtooth", "cited_reference_commit": "1d6dedb6cfb483c49d68429c5c6b328d44c0603e", "cited_reference_file": "validator/DAG/DAG_module.cpp", "cited_reference_scope": "earlier-to-later conflict DAG; no cycle-victim policy", "cycle_resolution": "deterministic cyclic-SCC victim; lowest original ordinal inside a cyclic SCC; no runtime feedback", "runtime_safety": "no elementary-cycle materialization"}),
    _manifest("scheduler", "cg_scheduler", "CG Scheduler", "Constructs the full directed RW/WR/WW conflict graph, resolves only cyclic SCCs with a deterministic original-ordinal victim rule, and then derives topological waves without enumerating elementary cycles.", capabilities=["consensus_bound_schedule", "full_pairwise_conflict_detection", "cycle_detection", "deterministic_cycle_abort", "topological_waves"], requirements=["execution:cg_execution", "block_executor:cg_block_executor"], supported_backends=["real_cluster"], truth_boundary="cg_cycle_aware_conflict_graph_abort_v4"),
    _manifest("block_executor", "cg_block_executor", "CG Block Executor", "Executes each acyclic conflict-graph frontier in parallel and terminalizes deterministic cycle victims as failed no-op outcomes.", config={"worker_count": 4}, schema=_schema({"worker_count": {"type": "integer", "minimum": 1, "maximum": 64, "default": 4}}), capabilities=["parallel_execution", "deterministic_materialization", "deterministic_cycle_abort_receipts"], supported_backends=["real_cluster"], truth_boundary="cg_cycle_aware_conflict_graph_abort_v4"),
