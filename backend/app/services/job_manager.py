...
class JobManager:
    ...
    def update_run(self, run_id: str, **updates: Any) -> dict[str, Any]:
        ...
        metadata["summary_available"] = (
            ...
            or (self.run_dir(run_id) / "metatrack_summary.csv").is_file()
        )
        metadata["report_available"] = (self.run_dir(run_id) / "report.md").is_file() or (self.run_dir(run_id) / "sweep_report.md").is_file() or (self.run_dir(run_id) / "calibration_report.md").is_file() or (self.run_dir(run_id) / "metatrack_ablation_report.md").is_file()
