...
function App() {
  ...
  return <main className="app-shell">
    <header><p className="eyebrow">元宇宙区块链实验平台</p><h1>V1-final 验收集成</h1><p className="muted">后端地址：{API_BASE_URL} · 保留 V0 默认实验，同时接入 V1.5/V1.6/V1.7/V1.8 验收入口。</p></header>
    ...
    <section className="panel" aria-labelledby="v1-status-title">
      ...
      <div className="status-grid">{v1Stages.map((stage) => <article key={stage.id} className="status-card"><strong>{stage.label}</strong><span>{stage.id}</span><b>{statusLabel(stage.status)}</b></article>)}</div>
    </section>
    <section className="panel v1-wizard" aria-labelledby="v1-title">
      ...
      <section className="wizard-step"><h3>Step 4：负载来源</h3><div className="wizard-grid"><article className="wizard-card selected"><strong>Synthetic workload</strong><span>当前插件：asset_hotspot / V1 workload trace enhancement</span><b>当前可用</b></article><article className="wizard-card selected"><strong>Existing trace replay</strong><span>复用已有 trace 进行 replay 与 V1.8 sweep。</span><b>当前可用</b></article><article className="wizard-card selected"><strong>Fabric chain-backed trace</strong><span>小规模真实链上 trace smoke runner。</span><b>已完成，CLI/WSL</b></article></div></section>
    ...
    <section className="panel" aria-labelledby="sweep-title">
      ...
      <div className="baseline-grid">{baselines.map(([name, routing, dualTrack, aggregation]) => <article key={name} className="baseline-card"><strong>{name}</strong><span>routing.policy = {routing}</span><span>dual_track_enabled = {dualTrack}</span><span>hot_update_aggregation_enabled = {aggregation}</span></article>)}</div>
      ...
      <ul className="file-list">{["report.md", "sweep_summary.csv", "sweep_summary.json"].map((filename) => { const file = sweepFiles.find((item) => item.name === filename); return <li key={filename}><span>{filename}</span><span className={file ? "file-present" : "file-missing"}>{file ? `${file.size_bytes} bytes` : "未生成"}</span>{file ? <a href={v1SweepFileDownloadURL(filename)}>下载</a> : <span>—</span>}</li>; })}</ul>
    </section>
    ...
    <section className="panel" aria-labelledby="fabric-title">
...
function MetricSection({ title, row, keys }: { title: string; row: V1SweepRow; keys: readonly string[] }) {
  return <section className="metric-section"><h4>{title}</h4><dl className="metrics-grid compact">{keys.map((key) => <div key={key}><dt>{key}</dt><dd>{formatMetric(row[key])}</dd></div>)}</dl></section>;
}
