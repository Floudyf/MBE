...
function App() {
  ...
  return <div className="final-shell">
    ...
    <main className="final-main">
      ...
      {activePage === "v5design" && <V5MethodDesignPage onDirtyChange={setDesignDirty} saveRequestToken={designSaveRequestToken} onSaveRequestHandled={(token) => setDesignSaveRequestToken((current) => current === token ? 0 : current)} onOpenRun={openRunWithMethod} />}
      ...
      {activePage === "runexperiment" && <V5FormalRunPage preferredMethodId={preferredMethodId} onPreferredMethodConsumed={() => { setPreferredMethodId(""); window.localStorage.removeItem("mbe.v5PreferredMethodId"); }} onPreferredMethodUnavailable={() => { setPreferredMethodId(""); window.localStorage.removeItem("mbe.v5PreferredMethodId"); }} onOpenResults={(groupId) => { setPreferredGroupId(groupId); window.localStorage.setItem("mbe.v5FormalRunGroupId", groupId); setActivePage("runs"); }} />}
