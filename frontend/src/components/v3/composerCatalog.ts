...
export const composerCatalog: Record<string, ModuleCatalogEntry> = {
  ...
  Consensus: {
    ...
    plugins: [
      ...
      { id: "blockemulator_aligned_pbft_preview", label: "BlockEmulator-aligned PBFT over NetworkAdapter preview", status: "runnable" },
  ...
  Routing: {
    ...
    plugins: [
      ...
      { id: "metatrack_coaccess_routing", label: "MetaTrack co-access routing", status: "runnable" },
      { id: "hotspot_aware_routing", label: "Hotspot-aware routing", status: "runnable" },
      { id: "co_access_sharding", label: "共访问路由", status: "runnable" },
      { id: "clpa_like_partitioning", label: "CLPA-like partitioning", status: "planned" },
      { id: "shardcutter_like_partitioning", label: "ShardCutter-like partitioning", status: "planned" },
      { id: "relay_cross_shard", label: "Relay cross-shard policy", status: "planned" },
      { id: "broker_cross_shard", label: "Broker cross-shard policy", status: "planned" },
      { id: "two_phase_commit", label: "2PC cross-shard policy", status: "planned" },
  ...
  Execution: {
    moduleId: "Execution",
    label: "交易执行",
    description: "执行排序后的交易，并决定串行、轻量并行、双轨或未来并行调度策略。",
    defaultPlugin: "serial_execution",
    required: true,
    allowVariable: true,
    plugins: [
      { id: "serial_execution", label: "串行执行", status: "runnable" },
      { id: "dual_track_execution", label: "双轨执行（兼容别名）", status: "runnable" },
      { id: "parallel_light_execution", label: "Parallel-light execution", status: "runnable" },
      { id: "metatrack_dual_track_execution", label: "MetaTrack dual-track light model", status: "runnable" },
      { id: "parallel_naive", label: "朴素并行", status: "planned" },
      { id: "block_stm_like", label: "Block-STM-like", status: "planned" },
      { id: "block_stm_like_model", label: "Block-STM-like model", status: "planned" },
      { id: "calvin_like_model", label: "Calvin-like model", status: "planned" },
      { id: "real_optimistic_execution", label: "Real optimistic execution", status: "planned" },
      { id: "real_rollback_engine", label: "Real rollback engine", status: "planned" },
      { id: "block_stm_like_planned", label: "Block-STM-like", status: "planned" },
    ],
    notes: ["Execution handles deterministic local scheduling, dependency estimates, and light fast/conservative track assignment. It does not implement real concurrent execution or rollback in V3.4.6."],
  },
  StateAccess: {
    moduleId: "StateAccess",
    label: "状态访问",
    description: "为交易执行提供状态读取路径，可比较直接读取、远程访问估计、缓存命中与访问列表预取。",
    defaultPlugin: "direct_fetch",
    required: true,
    allowVariable: true,
    plugins: [
      { id: "direct_fetch", label: "直接读取", status: "runnable" },
      { id: "remote_state_access_model", label: "Remote state access light model", status: "runnable" },
      { id: "cached_state_access", label: "Cached state access light model", status: "runnable" },
      ...
      { id: "real_witness_fetch", label: "Real witness fetch", status: "planned" },
      { id: "real_proof_fetch", label: "Real proof fetch", status: "planned" },
      { id: "mpt_proof_model", label: "MPT proof model", status: "planned" },
      { id: "persistent_kv_access", label: "Persistent KV access", status: "planned" },
      { id: "snapshot_access", label: "Snapshot access", status: "planned" },
    ],
    notes: ["StateAccess handles deterministic local/remote state access, cache/prefetch modeling, and proof/witness estimates. It does not implement real remote storage, proofs, witnesses, MPTs, state roots, persistent KV, or snapshots in V3.4.7."],
  },
  StateStorage: {
  ...
  Commit: {
    moduleId: "Commit",
    label: "状态提交",
    description: "提交执行结果，记录普通提交、保守提交、热点更新聚合与约束检查聚合的 deterministic light model。",
    defaultPlugin: "normal_commit",
    required: true,
    allowVariable: true,
    plugins: [
      { id: "normal_commit", label: "普通提交", status: "runnable" },
      { id: "conservative_commit", label: "Conservative commit light model", status: "runnable" },
      { id: "hot_update_aggregation", label: "Hot update aggregation light model", status: "runnable" },
      { id: "constraint_checked_aggregation", label: "Constraint-checked aggregation light model", status: "runnable" },
      { id: "hot_update_aggregation_commit", label: "热点更新聚合提交兼容别名", status: "runnable" },
      { id: "atomic_reservation_commit", label: "Atomic reservation commit", status: "planned" },
      { id: "batch_commit", label: "批量提交", status: "planned" },
      { id: "real_db_lock_commit", label: "Real DB lock commit", status: "planned" },
    ],
    notes: ["Commit handles deterministic local commit paths, hot-update aggregation, and constraint-check accounting. It does not implement real database locking, real concurrent commit, rollback, MPT/state root, persistent KV, or snapshots in V3.4.8."],
  },
  MetricsReport: {
