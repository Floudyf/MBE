...
export default function V5EvidencePanel({
  ...
  return <section className="v5-dashboard-section" data-testid="v5-evidence-panel">
    ...
    <section className="v5-dashboard-section" data-testid="v5-artifact-storage-panel">
      <div className="v5-dashboard-heading"><div><h4>产物存储管理</h4><p className="muted">Formal child 的结果、指标与证据索引冻结后自动冷归档为逐子实验 tar.zst。Windows 优先使用系统自带 tar.exe 流式打包，并由 Zstandard level 3 使用全部逻辑 CPU 线程压缩；原生 tar 不可用时自动回退 Python tar writer。完整校验通过后才释放 Raw。</p></div></div>
...
function storagePhaseText(value: unknown): string { return ({ queued: "排队", preparing: "准备", preparing_child: "准备当前 child", snapshotting: "冻结文件清单与哈希", packing: "原生 tar 打包", compressing: "Zstandard 多线程压缩", native_tar_fallback: "原生 tar 回退", checksumming_archive: "计算归档 SHA-256", verifying: "逐文件完整校验", releasing_raw: "释放 Raw", already_archived: "跳过已归档 child", child_finished: "当前 child 完成", completed: "全部完成", interrupted: "后端任务中断", failed: "任务失败" } as Record<string, string>)[String(value ?? "")] ?? String(value ?? "—"); }
function storageEngineText(value: unknown): string { return ({ windows_tar_zstd_multithread: "Windows tar.exe + Zstandard 多线程", python_tarfile_zstd_multithread: "Python tar + Zstandard 多线程" } as Record<string, string>)[String(value ?? "")] ?? String(value ?? "自动选择"); }
