...
export default function WorkloadSourceEditor({ state, datasets, onChange }: { state: WorkloadEditorState; datasets: V5WorkloadDatasetSummary[]; onChange: (state: WorkloadEditorState) => void }) {
  ...
  const skewAxes = ((dataset as { supported_skew_axes?: string[] } | undefined)?.supported_skew_axes ?? []).filter(Boolean);
  const defaultAxis = ((dataset as { default_skew_axis?: string | null } | undefined)?.default_skew_axis ?? skewAxes[0] ?? "contract");
  ...
  return <article className="final-card wide" data-testid="workload-source-editor">
    ...
    <div className="segmented-control" role="group" aria-label="负载来源">
      <button type="button" data-testid="workload-mode-synthetic" className={state.mode === "synthetic" ? "selected" : ""} onClick={() => patch({ mode: "synthetic", useFullDataset: false })}>模拟负载</button>
      <button type="button" data-testid="workload-mode-original" className={state.mode === "dataset_original" ? "selected" : ""} disabled={!hasSelectableDataset} onClick={() => patch({ mode: "dataset_original" })}>真实原始负载</button>
      <button type="button" data-testid="workload-mode-derived" className={state.mode === "dataset_derived" ? "selected" : ""} disabled={!hasSelectableDataset} onClick={() => patch({ mode: "dataset_derived", skewAxis: state.skewAxis || defaultAxis })}>真实派生偏斜负载</button>
    </div>
    {state.mode !== "synthetic" && <div className="experiment-condition-grid">
      ...
      <label><span>负载模式</span><input value={state.mode === "dataset_derived" ? "key_zipf" : "original_window"} readOnly /></label>
      {state.mode === "dataset_derived" && skewAxes.length > 1 && <label><span>偏斜轴</span><select aria-label="skew_axis" value={state.skewAxis || defaultAxis} onChange={(event) => patch({ skewAxis: event.target.value })}>{skewAxes.map((axis) => <option key={axis} value={axis}>{axis}</option>)}</select></label>}
      {state.mode === "dataset_derived" && skewAxes.length <= 1 && <label><span>偏斜轴</span><input aria-label="skew_axis" value={state.skewAxis || defaultAxis} readOnly /></label>}
      {state.mode === "dataset_derived" && <label><span>偏斜度 alpha</span><select aria-label="target_alpha" value={state.targetAlpha} onChange={(event) => patch({ targetAlpha: globalThis.Number(event.target.value) })}>{WORKLOAD_ALPHA_OPTIONS.map((value) => <option key={value} value={value}>{value.toFixed(1)}</option>)}</select></label>}
