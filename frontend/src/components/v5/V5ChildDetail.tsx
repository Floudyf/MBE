...
function mechanismRows(metrics: Record<string, unknown>): Row[] {
  return [
    ...
    ["规划调度事件数", metrics.planning_scheduler_event_count],
    ["运行时调度事件数", metrics.runtime_scheduler_event_count],
    ["Leader 调度事件数", metrics.leader_scheduler_event_count],
    ["Replica 调度事件数", metrics.replica_scheduler_event_count],
    ["唯一逻辑调度决策数", metrics.unique_logical_scheduling_decision_count],
    ...
    ["物理远程操作数", metrics.physical_remote_operation_count],
    ...
    ["副本去重远程操作数", metrics.replica_deduplicated_remote_operation_count],
    ...
    ["聚合前物理操作数", metrics.pre_aggregation_physical_op_count],
    ["聚合后物理操作数", metrics.post_aggregation_physical_op_count],
    ["聚合键数", metrics.aggregated_key_count],
    ["聚合逻辑增量数", metrics.aggregated_logical_delta_count],
    ["节省物理操作数", metrics.physical_ops_saved_count],
    ["聚合削减比例", metrics.aggregation_reduction_ratio],
    ["逻辑更新数（旧指标）", metrics.logical_update_count_deprecated ?? metrics.logical_update_count],
