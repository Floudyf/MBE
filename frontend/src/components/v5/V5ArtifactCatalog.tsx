...
export default function V5ArtifactCatalog({ groupId, catalog }: { groupId: string; catalog: V5FormalArtifactCatalog | null }) {
  ...
  return <section className="final-card wide" data-testid="v5-group-artifact-catalog"><h2>Group Artifact Catalog</h2>
    <dl className="stage-flow-kpis"><Metric label="Catalog status" value={catalog?.status} /><Metric label="Files" value={catalog?.file_count} /><Metric label="Bundle ready" value={catalog?.bundle_ready} /><Metric label="Bundle bytes" value={catalog?.bundle_size_bytes} /></dl>
...
function Metric({ label, value }: { label: string; value: unknown }) { return <div><dt>{label}</dt><dd>{value === undefined || value === null ? "-" : String(value)}</dd></div>; }
