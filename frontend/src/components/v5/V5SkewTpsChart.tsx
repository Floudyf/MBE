...
function ThetaMetricChart({ testId, title, unit, metric, series, thetaValues }: { testId: string; title: string; unit: string; metric: MetricKind; series: MetricSeries[]; thetaValues: number[] }) {
  ...
  return <article className="final-card wide analysis-chart" data-testid={testId}>
    <div className="section-heading"><div><h3>{title}</h3><p className="muted">横轴固定使用构造目标偏斜 θ（workload_point.target_theta）；单个随机种子时只跨重复运行统计，多个随机种子时先计算各随机种子的重复均值，再对这些均值计算 Student-t 95% 置信区间。{metric === "tps" ? " ○* 表示运行达到 60 分钟 hard timeout 后，尾部稳定性通过门禁并外推得到的预计完整工作负载 TPS；原始 child 仍保持失败/超时状态。" : " 延迟只使用完整实测样本，超时外推样本不会用于 P99。"}</p></div></div>
    <div className="table-wrap">
      <svg data-testid={`${testId}-svg`} viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${title} line chart`} style={{ width: "100%", minWidth: 720 }}>
        ...
        <text x={left + plotWidth / 2} y={height - 12} textAnchor="middle" fontSize="13">目标偏斜 θ</text>
