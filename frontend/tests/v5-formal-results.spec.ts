...
test("shows persisted V5 results, runtime evidence, and real artifacts", async ({ page, request }) => {
  ...
  await page.getByLabel("timeout_every").fill("0");
  ...
  await expect(page.getByTestId("v5-run-group-list").locator("tr", { hasText: groupId })).toContainText("completed");

  const row = page.getByTestId("v5-child-table").locator("tbody tr").first().locator("td");
  await expect(row.nth(7)).toHaveText("completed");
  ...
  const href = await childCatalog.locator("tr", { hasText: "finality_summary.json" }).getByRole("link", { name: "Download" }).getAttribute("href");
