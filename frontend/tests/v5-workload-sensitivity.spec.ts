...
import { openRunWithMethods, selectOnlySuite } from "./v5-formal-test-helpers";

test("workload sensitivity keeps every selected method for every workload point", async ({ page }) => {
    await openRunWithMethods(page, "hash_serial");
    await page.getByTestId("v5-run-method-metatrack_serial").locator("input[type='checkbox']").check();
    ...
    for (const id of ["hash_serial", "metatrack_serial"]) {
