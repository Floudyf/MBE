...
test("workload sensitivity keeps every selected method for every workload point", async ({ page, request }) => {
  ...
  try {
    ...
    await fields.nth(3).fill("40"); await fields.nth(4).fill("0.25"); await fields.nth(5).fill("0");
    ...
    for (const id of ["v5_catalog_default", method.config_id]) {
      ...
      expect(rows.map((row: { estimated_transactions: number }) => row.estimated_transactions).sort()).toEqual([20, 40]);
    }
    const pointA = body.rows.filter((row: { estimated_transactions: number }) => row.estimated_transactions === 20);
    const pointB = body.rows.filter((row: { estimated_transactions: number }) => row.estimated_transactions === 40);
